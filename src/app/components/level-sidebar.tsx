...
export function LevelSidebar({
  ...
  const currentLevelIndex = problemBank.blocks[currentBlockIndex]?.levels.findIndex(l => l.id === currentLevelId) ?? 0;
    ...
    <div className="h-full flex flex-col">
        ...
        <ScrollArea className="flex-1">
            ...
            {problemBank.blocks.map((block, blockIndex) => {
                ...
                return (
                <AccordionItem value={`block-${block.id}`} key={block.id} className="border-b-0 mb-2 rounded-lg bg-card overflow-hidden" disabled={!isBlockUnlocked}>
                    ...
                    {block.levels.map((level, levelIndex) => {
                        const isLevelUnlocked = isBlockUnlocked && (!isCurrentBlock || levelIndex <= currentLevelIndex);
                        return (
                            <div key={level.id} className={`flex flex-col pl-6 pr-2 py-1 ${!isLevelUnlocked ? 'opacity-50' : ''}`}>
                    ...
                    {block.checkpoint && (() => {
                        const isCheckpointUnlocked = isBlockUnlocked && (!isCurrentBlock || block.levels.length <= currentLevelIndex);
                        return (
                            <div className={`flex flex-col pl-6 pr-2 py-1 mt-2 ${!isCheckpointUnlocked ? 'opacity-50' : ''}`}>
